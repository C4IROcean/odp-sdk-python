...
def test_tabular_client(odp_client: OdpClient):
    ...
    my_dataset = odp_client.catalog.create(my_dataset)
    assert isinstance(my_dataset, ResourceDto)

    table_schema = {"Data": {"type": "string"}}
    my_table_spec = TableSpec(table_schema=table_schema)

    mt_table_spec = odp_client.tabular.create_schema(resource_dto=my_dataset, table_spec=my_table_spec)
    assert isinstance(mt_table_spec, TableSpec)

    test_data = [{"Data": "Test"}, {"Data": "Test1"}]
    odp_client.tabular.write(resource_dto=my_dataset, data=test_data)

    our_data = odp_client.tabular.select_as_list(my_dataset)
    assert our_data != []

    our_data = odp_client.tabular.select_as_stream(my_dataset)
    assert our_data != []

    update_filters = {"#EQUALS": ["$Data", "Test"]}
    new_data = [{"Data": "Test Updated"}]
    odp_client.tabular.update(
        resource_dto=my_dataset,
        data=new_data,
        filter_query=update_filters,
    )

    result = odp_client.tabular.select_as_list(my_dataset)
    assert result != []

    delete_filters = {"#EQUALS": ["$Data", "Test1"]}
    odp_client.tabular.delete(resource_dto=my_dataset, filter_query=delete_filters)
    result = odp_client.tabular.select_as_list(my_dataset)
    assert result != []

    odp_client.tabular.delete_schema(my_dataset)

    try:
        odp_client.tabular.get_schema(my_dataset)
    except OdpResourceNotFoundError as e:
        print("Schema not found error since it is deleted:")
        print(e)

    odp_client.catalog.delete(my_dataset)
