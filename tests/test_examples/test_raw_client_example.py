...
def test_raw_client(odp_client: OdpClient):
    ...
    file_dto = odp_client.raw.create_file(
    ...
    for file in odp_client.raw.list(my_dataset):
        print(file)
    assert odp_client.raw.list(my_dataset) != []

    # Download file
    odp_client.raw.download_file(my_dataset, file_dto, "test.txt")
    assert os.path.exists("test.txt")

    # Clean up
    odp_client.raw.delete_file(my_dataset, file_dto)
