...
def test_get_file_metadata_success(raw_storage_client, raw_resource_dto):
    ...
    with responses.RequestsMock() as rsps:
        rsps.add(
            responses.GET,
            f"{raw_storage_client.raw_storage_url}/{raw_resource_dto.metadata.uuid}/{file_ref}/metadata",
            body=file_meta.model_dump_json(),
            status=200,
            content_type="application/json",
        )

        result = raw_storage_client.get_file_metadata(raw_resource_dto, file_ref)
...
def test_get_file_metadata_not_found(raw_storage_client, raw_resource_dto):
    ...
    with responses.RequestsMock() as rsps:
        ...
        with pytest.raises(OdpFileNotFoundError):
            raw_storage_client.get_file_metadata(raw_resource_dto, file_ref)
...
def test_create_file_success(raw_storage_client, raw_resource_dto):
    ...
    with responses.RequestsMock() as rsps:
        ...
        rsps.add(
            responses.GET,
            f"{raw_storage_client.raw_storage_url}/{raw_resource_dto.metadata.uuid}/{file_name}/metadata",
            json=json.loads(file_metadata.model_dump_json()),
            status=200,
            content_type="application/json",
        )

        result = raw_storage_client.create_file(raw_resource_dto, file_meta=file_metadata, contents=None)
...
def test_download_file_save(raw_storage_client, raw_resource_dto, tmp_path):
    ...
    with responses.RequestsMock() as rsps:
        rsps.add(
            responses.GET,
            f"{raw_storage_client.raw_storage_url}/{raw_resource_dto.metadata.uuid}/{file_meta_data.name}",
            body=file_data,
            status=200,
        )

        raw_storage_client.download_file(raw_resource_dto, file_meta_data, save_path=str(save_path))
...
def test_delete_file_not_found(raw_storage_client, raw_resource_dto):
    ...
    with responses.RequestsMock() as rsps:
        ...
        with pytest.raises(OdpFileNotFoundError):
            raw_storage_client.delete_file(raw_resource_dto, filename)
